...
struct MutableWord {
  ...
  void patch(uint32_t Value, uint32_t Mask) {
    Wd = (Wd & ~Mask) | Value;
  }
...
TEST(AArch32_Relocations, Arm_Call_Bare) {
  ...
  for (MutableWord Mem : MemPresets) {
    ...
    EXPECT_EQ(EncodeDecode(1, Mem), 0);               // Zero value
    EXPECT_EQ(EncodeDecode(0x41, Mem), 0x40);         // Common value
    EXPECT_EQ(EncodeDecode(33554430, Mem), 33554428);   // Maximum value
    EXPECT_EQ(EncodeDecode(-33554432, Mem), -33554432); // Minimum value
    EXPECT_NE(EncodeDecode(33554433, Mem), 33554428);   // First overflow
    EXPECT_NE(EncodeDecode(-33554435, Mem), -33554432); // First underflow
